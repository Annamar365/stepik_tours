...
# Добавила сюда содержимое файла data.py. Возможно, есть варианты лучше, но я пока не нашла их в курсе
title = "Stepik Travel"
...
def main_view(request):
    ...
    main_data['subtitle'] = subtitle
    main_data['description'] = description

    ran_idx = []
    for it in range(1, len(tours)+1):
        ran_idx.append(it)
    ...
    max_ran_tours = 6
    for it in ran_idx:
        ran_tours[it] = tours[it]
        if len(ran_tours) == max_ran_tours:
            break

    context = {'main_data': main_data, 'ran_tours': ran_tours, 'departures': departures}
...
def departure_view(request, departure):
    ...
    for idx, tour in tours.items():
        if tour['departure'] == departure:
            curr_tours[idx] = tour
            tours_cnt = tours_cnt + 1
            prices.append(tour['price'])
            nights.append(tour['nights'])

    if tours_cnt == 1:
        cnt_label = "тур"
    elif tours_cnt >= 2 and tours_cnt <= 4:
        cnt_label = "тура"
    elif tours_cnt >= 5:
        cnt_label = "туров"

    tours_data = dict()
    tours_data['departure_text'] = departures[departure]
    tours_data['tours_cnt'] = tours_cnt
    tours_data['cnt_label'] = cnt_label
    tours_data['min_price'] = min(prices)
    tours_data['max_price'] = max(prices)
    tours_data['min_nights'] = min(nights)
    tours_data['max_nights'] = max(nights)

    context = {'tours_data': tours_data, 'curr_tours': curr_tours, 'departures': departures}
...
def tour_view(request, id):
    tour_data = tours[id]
    tour_data['departure_text'] = departures[tour_data['departure']]

    stars_num = int(tour_data['stars']) + 1
    stars_text = ""
    for it in range(1, stars_num):
        stars_text = stars_text + "★"

    tour_data['stars_text'] = stars_text

    context = {'tour': tour_data, 'departures': departures}
